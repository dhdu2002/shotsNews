...
def create_application() -> QApplication:
    ...
    application.setApplicationName("데일리 이슈 데스크톱")
    application.setOrganizationName("Daily Issue")
    return application


def create_runtime_viewmodel(desktop_app: DesktopApp | None = None) -> DashboardViewModel:
    """실제 DesktopApp 런타임에 연결된 기본 뷰모델을 만든다."""
    return DashboardViewModel(
        runtime_adapter=DesktopAppAdapter(desktop_app or DesktopApp()),
...
def create_main_window(
    ...
    desktop_app: DesktopApp | None = None,
) -> DashboardMainWindow:
    """대시보드 메인 윈도우를 만든다."""
